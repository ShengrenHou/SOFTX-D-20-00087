...
# First, create an environment using the factory's "generate_standard_environment" method. The environment automatically
# encapsulates time, weather and price data/information.
...
# All single-family houses are equipped with a fixed load, 20% have a deferrable load, and 30% have an electric vehicle.
# Moreover, 50% of all single-family houses have a battery unit and 80% have a rooftop photovoltaic unit installation.
...
# All apartments inside a multi-family houses are equipped with a fixed load, 20% have a deferrable load, and 20% have
# an electric vehicle. Moreover, 40% of all multi-family houses have a battery unit and 80% have a rooftop photovoltaic
# unit installation. The values are rounded in case they cannot be perfectly matched to the given number of buildings.
mfh_device_probs = {
    'FL': 1,
    'DL': 0.2,
    'EV': 0.2,
...
# The city district's maximum operational flexibility potential in terms of the absolute flexibility gain metric can be
# quantified very easily using the pycity_scheduling's "calculate_flexibility_potential" function. The absolute
# flexibility gain is a measure for the energy shifted between a coordinated and an uncoordinated scheduling scenario.
# Here, the coordinated case is represented through the central optimization algorithm, whereas the uncoordinated case
# is represented through the stand-alone optimization algorithm.
...
# Conclusions:
# In the given scenario, the absolute flexibility gain within the city district scenario is about 417kWh for the defined
...
# The quantification of the flexibility potential is usually a crucial task when it comes to the assessment of local
# energy systems and urban energy systems.
