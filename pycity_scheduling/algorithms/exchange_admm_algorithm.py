...
def exchange_admm(city_district, optimizer="gurobi_persistent", mode="convex", models=None, beta=1.0, eps_primal=0.1,
    ...
    while r_norms[-1] > eps_primal or s_norms[-1] > eps_dual:
        ...
        for node_id, node in nodes.items():
            ...
            obj += rho / 2 * pyomo.sum_product(entity.model.P_El_vars, entity.model.P_El_vars)
            obj += pyomo.sum_product(
                [rho * (- pv + xv + uv) for pv, xv, uv in zip(current_P_El_Schedule[node_id], x_, u)],
        ...
        # term)
        obj += rho / 2 * pyomo.sum_product(city_district.model.P_El_vars, city_district.model.P_El_vars)
        obj += pyomo.sum_product(
            [rho * (- pv - xv - uv) for pv, xv, uv in zip(current_P_El_Schedule[0], x_, u)],
            city_district.model.P_El_vars
        )
        obj = pyomo.Objective(expr=obj)
